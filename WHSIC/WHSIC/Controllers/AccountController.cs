...
namespace WHSIC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if(user==null)
            {
                ViewBag.m = "Incorrect username or password!";
            }
            ...
            if (user != null)
            {
                ...
                {
                    ViewBag.m = "Confirm your account before you can login, check the link that was sent on your email ";
                    return View();
                }
                else
                {
                    ...
                    else if (UserManager.IsInRole(user.Id, "Secretary"))
                    {
                        return RedirectToAction("Index", "ProgramUpload");
                    }
                    else if (UserManager.IsInRole(user.Id, "Treasure"))
                    {
                        return RedirectToAction("Index", "Expense");
                    }
                    else if (UserManager.IsInRole(user.Id, "PA"))
                    {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            {
                string name = null;
               var emp = eb.GetAllEmployee().Find(x => x.Email == model.Email);

                var don = db.GetAllDonor().Find(x => x.Email == model.Email);
                ...
                string html = "<table style=\"border: none; font-family: verdana, tahoma, sans-serif;\">" +
                              "<tr> " +
                                  "<td> <h3>Hi "+name+", </h3><p> Please reset your password by clicking <a href=\"" + callbackUrl + "\">here</a>" +
