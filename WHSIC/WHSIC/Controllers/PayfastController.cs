...
namespace WHSIC.Controllers
{
    public class PayfastController : Controller
    {
        ...
        public ActionResult ValidateCommand(decimal cost)
        {
            var model = (DonationViewModel)Session["ApptView"];
            bool useSandbox = Convert.ToBoolean(ConfigurationManager.AppSettings["IsSandbox"]);
            var payfast = new PayFastModel(useSandbox);
            payfast.m_payment_id = " 07e641ad-4b34-d18b-513e-488a1c817a5e";
            payfast.item_name = "Donation";
            payfast.amount = cost;
            return View(payfast);
            
        }

        public ActionResult RedirectFromPayfast()
        {
            return View();
         }

        public ActionResult CancelFromPayfast()
        {
            return View();
        }


        public FileStreamResult PrintExt()
        {


            string email = User.Identity.Name;
            DonorViewModel findDonor = donorb.GetAllDonor().Find(x => x.Email == email);
            DonationViewModel findDonation = (from d in donationb.GetAllDonation() where d.Donor_Name == email select d).Last();
            DonationTypeViewModel findDonationType = donationTypeb.GetAllDonationType().Find(x => x.DonationtypeID == findDonation.DonationtypeID);
            ...
            table.AddCell(findDonor.Name);
            table.AddCell("              ");
            table.AddCell("              ");
            table.AddCell("Donation For");
            table.AddCell(findDonationType.Description);
            table.AddCell("              ");
            table.AddCell("              ");
            table.AddCell("Donated As");
            table.AddCell("Other Member");
            table.AddCell("              ");
            table.AddCell("              ");
            table.AddCell("Date");
            table.AddCell(ReportDate(findDonation.Date));
            table.AddCell("              ");
            table.AddCell("              ");
            table.AddCell("Time");
            table.AddCell(findDonation.Date.ToShortTimeString());
            table.AddCell("              ");
            table.AddCell("              ");
            table.AddCell("Amount");
            table.AddCell(findDonation.Amount.ToString("R00.00"));
            table.AddCell("              ");
            table.AddCell("              ");
